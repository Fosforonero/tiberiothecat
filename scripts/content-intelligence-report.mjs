...
// ── §15 Opportunity Packs (manual trend intake — scan content-output/) ────────

section(15, 'Opportunity Packs Available (content-output/)')

function listOpportunityPacks(dir) {
  const out = []
  if (!fs.existsSync(dir)) return out
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    if (!entry.isFile()) continue
    const name = entry.name
    // Match opportunity-pack output: content-intelligence-<slug>-YYYY-MM-DD.json
    // Exclude the report itself: content-intelligence-report-YYYY-MM-DD.json
    if (!name.startsWith('content-intelligence-')) continue
    if (name.startsWith('content-intelligence-report-')) continue
    if (!name.endsWith('.json')) continue
    try {
      const data = JSON.parse(fs.readFileSync(path.join(dir, name), 'utf8'))
      if (data && data.type === 'opportunity-pack') {
        out.push({
          file: name,
          slug: data.slug ?? '',
          date: data.date ?? '',
          title: data.title ?? '',
          trendOrigin: data.trendOrigin ?? '',
          riskLevel: data.riskLevel ?? '',
          candidateCategory: data.candidateCategory ?? '',
          dilemmasEnCount: Array.isArray(data.dilemmasEN) ? data.dilemmasEN.length : 0,
          dilemmasItCount: Array.isArray(data.dilemmasIT) ? data.dilemmasIT.length : 0,
          saturationFlaggedCount: data.saturationFlaggedCount ?? 0,
          suggestedWorkflowPrimary: data.suggestedWorkflow?.primary ?? '',
        })
      }
...
const opportunityPacks = listOpportunityPacks(outDir)

console.log(`  Found ${opportunityPacks.length} opportunity pack(s) in content-output/`)
for (const p of opportunityPacks.slice(0, 10)) {
  console.log(`    ${p.date}  ${p.slug.padEnd(28)} risk:${p.riskLevel.padEnd(6)} workflow:${p.suggestedWorkflowPrimary.padEnd(12)} EN:${p.dilemmasEnCount} IT:${p.dilemmasItCount} flagged:${p.saturationFlaggedCount}`)
}
if (opportunityPacks.length > 10) console.log(`    … ${opportunityPacks.length - 10} more in JSON`)
...
const report = {
  ...
  landingPageCandidates: landingCandidates,
  localeBalance: {
...
const md = [
  ...
  '',
  '## Opportunity Packs Available',
  opportunityPacks.length === 0
    ? '_None found in `content-output/`._  Use `npm run content:opportunity-pack -- <input.json>` to generate one.'
    : `${opportunityPacks.length} pack(s) found. Each is read-only — requires editorial review before any admin draft / seed batch.`,
  ...(opportunityPacks.length === 0 ? [] : [
    '',
    '| Date | Slug | Risk | Workflow | EN | IT | Flagged |',
    '|---|---|---|---|---|---|---|',
    ...opportunityPacks.slice(0, 12).map(p =>
      `| ${p.date} | \`${p.slug}\` | ${p.riskLevel} | \`${p.suggestedWorkflowPrimary}\` | ${p.dilemmasEnCount} | ${p.dilemmasItCount} | ${p.saturationFlaggedCount} |`
    ),
  ]),
  '',
  '---',
