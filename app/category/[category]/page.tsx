...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title,
    description,
    alternates: { canonical: `https://splitvote.io/category/${cat.value}` },
    openGraph: {
      title,
      description,
      url: `https://splitvote.io/category/${cat.value}`,
      siteName: 'SplitVote',
    },
...
export default async function CategoryPage({ params }: Props) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-16">
      {/* Breadcrumb */}
      <nav className="text-xs text-[var(--muted)] mb-8">
      ...
      {/* Grid — pre-filtered, no category buttons needed */}
